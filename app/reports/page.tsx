...
export default function ReportsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Late report: last 15 days
      const { data: att15 } = await supabase
        .from('attendance')
        .select('employee_id, work_date, check_in_time, employees(name)')
      ...
      // Branch cross report: last 30 days
      ...
      for (const a of att30 ?? []) {
        ...
        if (a.branch_id && primaryId && a.branch_id !== primaryId) {
          ...
          cross.push({
            name: emp?.name ?? '-',
            work_date: a.work_date,
            branch_name: (a.branches as any)?.name ?? a.branch_id,
            primary_branch: emp?.branches?.name ?? '-',
            is_ot: sched?.is_ot ?? false,
          })
        }
      }
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => setTab('late')} className={`flex-1 py-2 rounded-lg text-sm font-medium ${tab === 'late' ? 'bg-gray-900 text-white' : 'bg-white border text-gray-600'}`}>
            ⏰ มาสาย (15 วัน)
          </button>
